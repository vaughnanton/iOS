...
// Can use UserDefaults to store any basic data type for as long as the app is installed (bool, float, doublt, int, string, URL, arrays, dictionaries etc)
// don't use UserDefaults unless it's less than 100KB of data
...
class ViewController: UICollectionViewController, UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    ...
    override func viewDidLoad() {
        ...
        // this is effectively the save method in reverse
        // pull out optional Data, using if let and as? to unwrap it
        if let savedPeople = defaults.object(forKey: "people") as? Data {
            // give it to unarchive method to convert it back to an object graph (our array of Person objects)
    ...
    // save method
    func save() {
        // convert array into a Data object
        if let savedData = try? NSKeyedArchiver.archivedData(withRootObject: people, requiringSecureCoding: false) {
            // save data object to userdefaults, now just need to call this method whenever we change a name or add an image
            let defaults = UserDefaults.standard
